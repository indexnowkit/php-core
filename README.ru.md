...
Индексы sitemap раскрываются (только тот же host, 3 уровня, 1000 документов), `.gz` распаковывается с лимитом 50 МиБ,
...
всех отправляемых host (файл ключа всё равно нужен на каждом). Реализуйте `KeyProviderInterface`, чтобы брать ключи
из базы или реестра тенантов.

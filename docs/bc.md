...
the 403 counter in the failure cache; `History\Adapter\HistoryServices::forbiddenCounter()` needs it), `Check\DebounceStoreCheck::PROBE_KEY`
(the key every adapter's cache probe writes), `Check\LocalesCheck::CODE`, `KeyValidator::MIN_LENGTH`,
